...
# elif defined BICHAN
      DO k=1,N(ng)
       DO j=JstrT,JendT
         DO i=IstrP,IendT
	    val1 = xr(i,j)*6.283*1.0E-05
	    val2 = yr(i,j)*6.283*1.0E-05
            u(i,j,k,1)=(h(i,j)+z_r(i,j,k))*0.01_r8+0.05*SIN(val1)*SIN(val2)
          END DO
        END DO
        DO j=JstrP,JendT
          DO i=IstrT,IendT
...
# elif defined BICHAN
      DO k=1,N(ng)
        DO j=JstrT,JendT
          DO i=IstrT,IendT
            t(i,j,k,1,itemp)=T0(ng)+z_r(i,j,k)/(9.8_r8*1.7_r8)
            t(i,j,k,1,isalt)=S0(ng)+yr(i,j)/(9.8_r8*760.0_r8)
          END DO
        END DO
      END DO
